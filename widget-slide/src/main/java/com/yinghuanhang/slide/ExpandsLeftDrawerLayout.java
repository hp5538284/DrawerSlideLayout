 ...
 */
public class ExpandsLeftDrawerLayout extends HorizonLeftDrawerLayout {
    public ExpandsLeftDrawerLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public ExpandsLeftDrawerLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public ExpandsLeftDrawerLayout(Context context) {
        super(context);
    }
    ...
    public void onTouchUp(MotionEvent event) {
        ...
        if (getDrawerState() == DRAWER_OPEN) {      // 打开状态：移动距离有效则展开，右移但无效则恢复打开状态
            if (event.getX() - helper.getPosition() > area) {
                helper.setTarget(getMaxMargin());
                helper.setStart(params.leftMargin, 5);
                onRunning(true);
